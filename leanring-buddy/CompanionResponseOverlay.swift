...
final class CompanionResponseOverlayManager {
    ...
    private var cursorFollowingTimer: Timer?
    ...
    private func startCursorFollowing() {
        // 60fps cursor following so the panel stays glued to the mouse
        cursorFollowingTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true) { [weak self] _ in
    ...
    private func stopCursorFollowing() {
        cursorFollowingTimer?.invalidate()
        cursorFollowingTimer = nil
    }
