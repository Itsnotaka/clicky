...
//  leanring-buddy
//
//  Shared push-to-talk shortcut matching for the global keyboard monitor.
//
...
enum BuddyPushToTalkShortcut {
    ...
    private static func shortcutTransition(
        ...
        let hasRequiredModifiers = modifierFlags.contains(pushToTalkModifierFlags)
        let isPushToTalkKey = keyCode == pushToTalkKeyCode
        guard hasRequiredModifiers && isPushToTalkKey else {
            if wasShortcutPreviouslyPressed && shortcutEventType == .keyUp {
                return .released
            }
            return .none
        }
        ...
        case .keyDown where !wasShortcutPreviouslyPressed:
            return .pressed
        case .keyUp where wasShortcutPreviouslyPressed:
            return .released
        default:
            return .none
        }
